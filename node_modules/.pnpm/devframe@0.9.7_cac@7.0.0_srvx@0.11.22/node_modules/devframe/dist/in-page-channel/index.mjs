...
function createPageScriptChannel(options) {
	...
	for (const definition of options.functions ?? []) registry.register(definition);
...
function connectPanelChannel(options) {
	...
	function enqueueCall(method, args) {
		if (status === "closed") return Promise.reject(new InPageChannelError("closed", `in-page channel "${name}": call "${method}" rejected — the channel is closed`));
	...
	function handleDisconnect(reason) {
		...
		else warnOnce(`in-page channel "${name}": transport lost (${reason}) and the panel has no handshake targets — staying disconnected`);
	}
	...
	else warnOnce(`in-page channel "${name}": the panel has no handshake targets (not embedded, no opener) and no transport — calls will buffer until a transport appears or the channel is closed`);
	return {
		name,
		get status() {
			return status;
