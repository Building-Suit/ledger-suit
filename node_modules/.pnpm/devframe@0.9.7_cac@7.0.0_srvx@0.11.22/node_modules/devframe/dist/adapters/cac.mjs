...
import { a as importRuntimeModule } from "../instance-shell-CpVW4zG0.mjs";
import { createBuild } from "./build.mjs";
import { t as createDevServer } from "../dev-Czit5IP4.mjs";
...
function createCac(d, options = {}) {
	...
	const devCommand = cli.command("[...args]", "Start a local dev server").option("--port <port>", "Port to listen on").option("--host <host>", "Host to bind to", { default: defaultHost }).option("--open", "Open the browser on start").option("--no-open", "Do not open the browser").option("--no-auth", "Disable the interactive authentication gate").option("--mcp", "Expose an MCP server over HTTP at /__mcp (use --no-mcp to disable)");
