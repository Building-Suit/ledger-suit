...
<template>
  <div data-testid="section-skeleton" aria-busy="true" aria-live="polite">
    ...
    <div v-if="variant === 'cards'" class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <div v-for="index in 8" :key="index" class="ls-card space-y-4 p-5">
    ...
    <div v-else-if="variant === 'chart'" class="ls-card space-y-5 p-6">
    ...
    <div v-else class="ls-card overflow-hidden">
      <div class="flex items-center justify-between border-b border-[var(--bs-border)] px-5 py-4">
        <div class="ls-skeleton h-4 w-40 rounded-full" />
        <div class="ls-skeleton h-4 w-20 rounded-full" />
      </div>
      <div v-for="index in rowIndexes" :key="index" class="grid grid-cols-4 gap-5 border-b border-[var(--bs-border)] px-5 py-4 last:border-b-0">
