    ...
    <Teleport to="body">
      <div v-if="open" class="fixed inset-0 z-[70] grid place-items-center ls-scrim p-4" role="dialog" aria-modal="true" @click.self="close">
        <form class="ls-card w-full max-w-lg space-y-4 p-6" @submit.prevent="invite">
          ...
          <button class="ls-btn ls-btn-primary w-full" :disabled="pending">{{ t('org.createInvite') }}</button>
          <div v-if="token" class="rounded-control bg-surface-muted p-3">
            <p class="text-sm font-semibold">{{ t('org.shareToken') }}</p>
            <code class="mt-2 block break-all text-xs" dir="ltr">{{ token }}</code>
          </div>
