    ...
    <main>
      <section class="ls-landing-hero relative overflow-hidden border-b border-brand-gold">
        ...
        <div class="relative mx-auto grid max-w-7xl items-center gap-12 px-4 py-16 lg:grid-cols-[1.05fr_.95fr] lg:px-8 lg:py-24">
          <div class="max-w-3xl">
            <p class="mb-6 text-xs font-bold uppercase tracking-[.22em] text-brand-gold-highlight">{{ t('landing.eyebrow') }}</p>
            <h1 class="text-4xl font-black leading-[1.08] tracking-[-.055em] sm:text-6xl">{{ t('landing.heroTitle') }}</h1>
            <p class="ls-brand-hero-muted mt-6 max-w-2xl text-lg leading-8">{{ t('landing.heroBody') }}</p>
            <div class="mt-8 flex flex-wrap gap-3">
              <NuxtLink to="/signup" class="ls-btn ls-btn-accent">{{ t('landing.createWorkspace') }} <AppIcon name="arrowRight" directional /></NuxtLink>
              <a href="#features" class="ls-btn ls-btn-on-brand">{{ t('landing.explore') }}</a>
            </div>
            <p class="ls-brand-hero-muted mt-4 text-xs">{{ t('landing.trialNote') }}</p>
          </div>
      ...
      <section id="features" class="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        ...
        <div class="mt-12 grid gap-px overflow-hidden rounded-modal border border-[var(--bs-border)] bg-[var(--bs-border)] md:grid-cols-2 lg:grid-cols-3">
          <article v-for="feature in features" :key="feature.key" class="bg-surface p-8">
            ...
            <h3 class="mt-6 text-lg font-bold">{{ t(`landing.features.${feature.key}.title`) }}</h3>
