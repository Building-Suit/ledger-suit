...
export async function sendEmail(input: EmailInput): Promise<string> {
  ...
  const safeGreeting = escapeHtml(input.greeting ?? `Hello ${input.recipientName ?? input.to},`)
  const safeFooter = escapeHtml(input.footer ?? 'For your security, this invitation is tied to your email address and expires after 14 days. If you were not expecting it, you can safely ignore this message.')
